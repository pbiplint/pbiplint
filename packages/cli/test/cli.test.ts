...
describe("pbiplint CLI", () => {
  it("lints the sample project and exits 1 because it has errors", async () => {
    ...
    expect(r.out).toMatch(
      /^pbiplint: 185 findings \(16 errors, 54 warnings, 115 info\) in 14 files/,
    );
  ...
  it("summarizes on stderr when the report goes to --output, naming the file as typed", async () => {
    ...
    expect(r.out).toBe("");
    expect(r.err).toBe(
      "pbiplint: 185 findings (16 errors, 54 warnings, 115 info) in 14 files, wrote out/report.sarif\n",
    );
  });
