#!/usr/bin/env node
// Usage: npm run build -w @pbiplint/core && node scripts/sync-rule-pages.mjs
//
// Copies prose from the rule pages into generated data files, so packages that cannot read
// files at run time still speak in pbiplint's own words:
//   packages/core/src/rules/rule-summaries.data.ts  the first paragraph of "What it checks",
//                                                    which becomes Rule.description
//   packages/cli/src/rule-help.data.ts               the Example, Why, How to fix, When to ignore,
//                                                    and Quirks sections plus a link to the page,
//                                                    which the CLI hands to the SARIF formatter as
//                                                    each rule's help block
// The rule-pages tests fail when a data file and the pages disagree. Core is imported for the
// ignore mechanics, so build it first.
import { execFileSync } from "node:child_process";
import { readdirSync, readFileSync, realpathSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
import { ignoreHelp } from "@pbiplint/core";

const SUMMARIES_OUT = "packages/core/src/rules/rule-summaries.data.ts";
const HELP_OUT = "packages/cli/src/rule-help.data.ts";
const RULE_URL_BASE = "https://pbiplint.com/rules/";

/** The page's `## ` sections by heading, frontmatter and title dropped. */
export const sections = (text) => {
  const body = text.replace(/^---\n[\s\S]*?\n---\n/, "");
  const out = {};
  for (const chunk of body.split(/^## /m).slice(1)) {
    const nl = chunk.indexOf("\n");
    out[chunk.slice(0, nl).trim()] = chunk.slice(nl + 1).trim();
  }
  return out;
};

export const firstParagraph = (s) =>
  s
    .trim()
    .split(/\n\s*\n/)[0]
    ?.replace(/\s+/g, " ")
    .trim() ?? "";

/**
 * The Example section for the help block: each fence captioned, its info string reduced to `tmdl`
 * or, for a `pbir` fence, to `json`, with the file the document stands for in the caption. A
 * `tree.json` document names its files by its keys, so its caption stays bare, as on the site. On
 * a page whose example runs under a config, the `json pbiplint.config.json` fence that holds it is
 * captioned with the file name and reduced to `json`, so a Markdown reader gets a plain JSON fence
 * rather than an info string it does not know.
 */
export const exampleMarkdown = (example) =>
  example
    .replace(/^```json pbiplint\.config\.json[ \t]*$/gm, "**pbiplint.config.json**\n\n```json")
    .replace(/^```tmdl fires[ \t]*$/gm, "**Fires the rule**\n\n```tmdl")
    .replace(/^```tmdl fixed[ \t]*$/gm, "**After the fix**\n\n```tmdl")
    .replace(
      /^```pbir fires (\S+)[ \t]*$/gm,
      (_line, file) =>
        `**Fires the rule${file === "tree.json" ? "" : ` in ${file}`}**\n\n\`\`\`json`,
    )
    .replace(
      /^```pbir fixed (\S+)[ \t]*$/gm,
      (_line, file) =>
        `**After the fix${file === "tree.json" ? "" : ` in ${file}`}**\n\n\`\`\`json`,
    );

/**
 * Markdown help for SARIF consumers: the page minus its What section, in the page's order, with
 * the ignore mechanics generated onto the end of "When to ignore it", plus a link back.
 */
export const helpMarkdown = (s, url, id, scope) =>
  [
    ...(s["Example"] ? ["### Example", "", exampleMarkdown(s["Example"]), ""] : []),
    "### Why it matters",
    "",
    s["Why it matters"],
    "",
    "### How to fix it",
    "",
    s["How to fix it"],
    ...(s["When to ignore it"]
      ? ["", "### When to ignore it", "", s["When to ignore it"], "", ignoreHelp(id, scope)]
      : []),
    ...(s["Quirks"] ? ["", "### Quirks", "", s["Quirks"]] : []),
    "",
    `Read more: ${url}`,
  ].join("\n");

/**
 * A code span and its delimiters: a run of backticks, the text up to the next run of exactly that
 * length, and nothing longer on either side. Backticks that open no span, such as the ``` a
 * sentence names, match nothing here and are left where they are.
 */
const CODE_SPAN = /(?<!`)(`+)(?!`)([\s\S]*?)(?<!`)\1(?!`)/g;

/** The same help as plain text: headings kept as lines, code fences dropped, bold markers dropped, code spans unwrapped. */
export const helpText = (markdown) =>
  markdown
    .split("\n")
    .filter((line) => !/^```/.test(line))
    .map((line) => line.replace(/^### /, ""))
    .join("\n")
    .replace(/\*\*([^*\n]+)\*\*/g, "$1")
    .replace(/\[([^\]]+)\]\(([^)]+)\)/g, "$1 ($2)")
    .replace(CODE_SPAN, (_span, _ticks, body) =>
      // A span padded on both sides gives one space back, as a Markdown reader does, so that
      // `` `x` `` reads as `x` and not as a space, a backtick, x, a backtick, a space.
      body.startsWith(" ") && body.endsWith(" ") && body.trim() !== "" ? body.slice(1, -1) : body,
    );

function main() {
  const summaries = {};
  const help = {};
  for (const file of readdirSync("rules")
    .filter((f) => f.endsWith(".md"))
    .sort()) {
    const text = readFileSync(`rules/${file}`, "utf8");
    const frontmatter = /^---\n([\s\S]*?)\n---\n/.exec(text)?.[1] ?? "";
    const id = /^id: (.+)$/m.exec(frontmatter)?.[1]?.trim();
    if (!id) throw new Error(`${file}: no id in frontmatter`);
    const scope = (/^scope: \[(.*)\]$/m.exec(frontmatter)?.[1] ?? "")
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);
    const s = sections(text);
    for (const heading of ["What it checks", "Why it matters", "How to fix it"])
      if (!s[heading] || s[heading].startsWith("TODO"))
        throw new Error(`${file}: "${heading}" is not written`);
    summaries[id] = firstParagraph(s["What it checks"]);
    const markdown = helpMarkdown(s, RULE_URL_BASE + file.replace(/\.md$/, ""), id, scope);
    help[id] = { text: helpText(markdown), markdown };
  }

  writeFileSync(
    SUMMARIES_OUT,
    `// Generated by scripts/sync-rule-pages.mjs from the first paragraph of each rule page's
// "What it checks" section. Do not edit by hand: edit the page under rules/ and rerun the script.

/** What each rule checks, one paragraph in pbiplint's own words, keyed by rule id. */
export const RULE_SUMMARIES: Readonly<Record<string, string>> = ${JSON.stringify(summaries, null, 2)};
`,
  );
  writeFileSync(
    HELP_OUT,
    `// Generated by scripts/sync-rule-pages.mjs from the Example, Why, How to fix, When to ignore,
// and Quirks sections of each rule page. Do not edit by hand: edit the page under rules/ and
// rerun the script.
import type { RuleHelp } from "@pbiplint/core";

/** SARIF help per rule id: the page's Example, Why, How to fix, When to ignore, and Quirks sections, plus a link to the page. */
export const RULE_HELP: Readonly<Record<string, RuleHelp>> = ${JSON.stringify(help, null, 2)};
`,
  );
  execFileSync("node_modules/.bin/prettier", ["--write", SUMMARIES_OUT, HELP_OUT], {
    stdio: "ignore",
  });
  console.log(
    `wrote ${Object.keys(summaries).length} summaries to ${SUMMARIES_OUT} and ${Object.keys(help).length} help entries to ${HELP_OUT}`,
  );
}

// Only when run as a script, so a test can import the helpers above without rewriting the data
// files. argv[1] is realpathed first because Node realpaths the ESM main and not argv[1], so
// invoking this through a symlink would leave the two spellings unequal and skip main(), and a
// sync that wrote nothing would look like one that had nothing to write.
if (process.argv[1] && import.meta.url === pathToFileURL(realpathSync(process.argv[1])).href)
  main();
