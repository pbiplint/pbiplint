...
- Both columns of a relationship, the columns that row-level and object-level security name, and the default column of a variation are reached whether or not the report uses them, because the model needs them. A column that a reached column sorts by is reached too.
