...
- While a visual.json cannot be read, such as one holding merge-conflict markers, the rule reports nothing, because the visual in it could be of any registered type and pbiplint does not guess what a file it could not read says. The skipped line gives the reason, `a report file could not be read`, the file's own `PARSE_ISSUE` finding names it, and the Visuals line of Report at a glance says how many registered types are used is unknown.
