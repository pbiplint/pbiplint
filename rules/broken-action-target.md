...
- A page or a bookmark whose own file cannot be read, such as a page.json or a bookmark file holding merge-conflict markers or one pbiplint could not open at all, is not reported missing, because pbiplint does not guess what a file it could not read says. The page is known by its folder name and the bookmark by its file name, `<name>.bookmark.json`, which Microsoft's PBIR documentation says are a page's and a bookmark's `name` by default, so an action whose destination names one of them is not reported. Nor is a destination that a folder under the definition folder, which pbiplint could not list, could hold: any page while the pages folder could not be listed, and any bookmark while the bookmarks folder could not be. The file's own `PARSE_ISSUE` finding names it, or a notice does for a file pbiplint could not open or a folder it could not list.
