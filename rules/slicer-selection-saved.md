...
A custom slicer from AppSource sets and clears its own selection, as Microsoft's guidance for building Power BI visuals describes, so the controls for clearing it are the visual's own. In Power BI Desktop, clear it on the visual the way it offers, or with a setting in the Format pane where the visual has one, then save the report in that state. Check any bookmark that captures it as well, since the same guidance says that when a reader switches bookmarks, Power BI hands the visual the filter the bookmark holds.

In visual.json, the selection is the `filter` property under `objects.general[0].properties`, on a custom slicer as on Microsoft's: remove it, as the example does, and leave the rest of `general` as it is.
...
- A selection is read by where it sits, not by the visual's type, so custom slicers from AppSource, such as the Chiclet Slicer, the Hierarchy Slicer, and the Text Filter, are reported the same way as Microsoft's. Microsoft's guidance for building Power BI visuals says a visual that filters others declares a `filter` in the `general` section of its capabilities, and Power BI Desktop's saved files keep these slicers' selections in that `filter` under `objects.general`. In those files, every kind of visual that carries that `filter` is one that filters the report.
